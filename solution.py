...
def receiveOnePing(mySocket, ID, timeout, destAddr):
    ...
    while 1:
        ...
        read_header = recPacket[20:28]
        type, code, checksum, identifier, sequence = struct.unpack('bbHHh', read_header)
...
def sendOnePing(mySocket, destAddr, ID):
    ...
    myChecksum = checksum(header + data) ###############CHECK THIS FIELD
...
def ping(host, timeout=1):
    ...
    packet_min = min(ping_delays)
    packet_avg =  sum(ping_delays)/len(ping_delays)
    packet_max = max(ping_delays)
    stdev_var = statistics.stdev(ping_delays)
